...
def animeworld_scraper(url:str) -> list[list[str]] | None:
    ...
    episodes_download_link_list: list[list[str]] =[]
    for episode_url in episodes_url_list:
        ...
        episodes_download_link_list.append([link, file_name])
    
    logger.info("Scraping ended for Animeworld")
    return episodes_download_link_list
...
def stage_1_animeworld_scraper(url:str) -> list[str] | None:
    ...
    for tag in episodes_tags_list:
        if isinstance(tag, Tag) and isinstance(tag.a, Tag):
            link: str = str(tag.a.get("href"))
            episodes_download_links_list.append(uri+link)
        else:
            logger.error("episodes_tags_list did not contained was not type Tag. Check the site for changes in HTML")
            return None
...
def stage_2_animeworld_scraper(url:str) -> str | None:
    ...
    if not isinstance(download_link, Tag):
        logger.error("Stage 2 scraper did not returned Type Tag. Check the site for changes in HTML")
        return None
